...
public abstract class MultiTypeAdapter<V> extends TypeAdapter {
     ...
     * @return this adapter
     */
    public MultiTypeAdapter addItems(final int type, final Collection<V> items) {
     ...
     * @param type
     */
    protected abstract void update(int position, V item, int type);

    @Override
    public GenericVH onCreateViewHolder(ViewGroup viewGroup, int type) {
        return new GenericVH(initialize(type, inflater.inflate(layout[type], null)));
    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder genericVH, int position) {
        update(position, genericVH.itemView, getItem(position), getItemViewType(position));
    }
}
